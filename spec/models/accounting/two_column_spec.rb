RSpec.describe Accounting::TwoColumn do
  context '#==' do
    it 'returns true when credit and debit are same' do
      debit_value = 69_000.00.to_d
      credit_value = 42_000.00.to_d
      lhs = Accounting::TwoColumn.new(debit_value, credit_value)
      rhs = Accounting::TwoColumn.new(debit_value, credit_value)
    ...
    it 'returns false when credit is different' do
      ...
      lhs = Accounting::TwoColumn.new(debit_value, lhs_credit)
      rhs = Accounting::TwoColumn.new(debit_value, rhs_credit)
    ...
    it 'returns false when debit is different' do
      ...
      credit_value = 42_000.00.to_d
      lhs = Accounting::TwoColumn.new(lhs_debit, credit_value)
      rhs = Accounting::TwoColumn.new(rhs_debit, credit_value)
      expect(lhs).not_to eq(rhs)
    end
  end
  ...
  context '#abs_credit' do
    it 'returns the difference of credit from the debit' do
      debit_value = 69_000.00.to_d
      credit_value = 42_000.00.to_d
      val = Accounting::TwoColumn.new(debit_value, credit_value)
  ...
  context '#+' do
    it 'returns the correct value' do
      ...
      lhs = Accounting::TwoColumn.new(lhs_debit, lhs_credit)

      rhs_debit = 50_000.00.to_d; rhs_credit = 15_000.00.to_d
      rhs = Accounting::TwoColumn.new(rhs_debit, rhs_credit)
